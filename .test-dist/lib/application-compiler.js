...
exports.materializeTailoredResume = materializeTailoredResume;
exports.summarizeChanges = summarizeChanges;
exports.renderAtsText = renderAtsText;
exports.renderPrintableHtml = renderPrintableHtml;
exports.compileApplicationArtifacts = compileApplicationArtifacts;
...
function proposalMatchesSource(source, proposal) {
    if (!sameText(source.company, proposal.company) || !sameText(source.title, proposal.title)) {
        return false;
    }
    if (source.bullets.length !== proposal.bullets.length)
        return false;
...
function materializeTailoredResume(source, tailored) {
    const usedProposalIndexes = new Set();
    const findProposal = (experience, sourceIndex) => {
        ...
        const matches = tailored.tailoredExperience
            ...
            .filter(({ proposal, index }) => !usedProposalIndexes.has(index) && proposalMatchesSource(experience, proposal));
        if (matches.length !== 1)
            return undefined;
...
function summarizeChanges(source, compiled) {
    ...
    const skillsAdded = compiled.skills.filter((value) => !sourceSkills.has(normalize(value).toLowerCase()));
    const skillsRemoved = source.skills.filter((value) => !compiledSkills.has(normalize(value).toLowerCase()));
    const experienceBulletsChanged = [];
...
function renderAtsText(resume) {
    const lines = [];
    const contact = resume.contact;
    lines.push(normalize(contact.name));
    lines.push([contact.email, contact.phone, contact.location, contact.website]
        .map((value) => normalize(value || ""))
        .filter(Boolean)
        .join(" | "));
    if (resume.summary)
        lines.push("", "SUMMARY", normalize(resume.summary));
    if (resume.skills.length)
        lines.push("", "SKILLS", resume.skills.map(normalize).join(", "));
    if (resume.experience.length) {
        lines.push("", "EXPERIENCE");
        for (const entry of resume.experience) {
            lines.push([normalize(entry.title), normalize(entry.company)].filter(Boolean).join(" — "));
    ...
    if (resume.projects.length) {
        ...
        for (const project of resume.projects) {
            ...
            if (project.description)
                lines.push(normalize(project.description));
    ...
    if (resume.education.length) {
        ...
        for (const entry of resume.education) {
            lines.push([entry.degree, entry.fieldOfStudy, entry.institution]
                .map((value) => normalize(value || ""))
                .filter(Boolean)
                .join(" — "));
            if (entry.graduationDate)
                lines.push(normalize(entry.graduationDate));
        }
    }
...
function renderPrintableHtml(resume) {
    const section = (title, body) => body ? `<section><h2>${escapeHtml(title)}</h2>${body}</section>` : "";
    const experience = resume.experience
        .map((entry) => `<article><h3>${escapeHtml(entry.title)} · ${escapeHtml(entry.company)}</h3><p>${escapeHtml([entry.startDate, entry.endDate].filter(Boolean).join(" – "))}</p><ul>${entry.bullets
    ...
    const projects = resume.projects
        ...
        const technologies = entry.technologies.length
            ...
            : "";
        return `<article><h3>${escapeHtml(entry.name)}</h3>${entry.description ? `<p>${escapeHtml(entry.description)}</p>` : ""}<ul>${entry.bullets
            .map((bullet) => `<li>${escapeHtml(bullet)}</li>`)
            .join("")}</ul>${technologies}</article>`;
    })
        .join("");
    const education = resume.education
        .map((entry) => `<article><h3>${escapeHtml(entry.institution)}</h3><p>${escapeHtml([entry.degree, entry.fieldOfStudy].filter(Boolean).join(" · "))}</p>${entry.graduationDate ? `<p>${escapeHtml(entry.graduationDate)}</p>` : ""}</article>`)
        .join("");
    const certifications = resume.certifications.length
        ? `<ul>${resume.certifications
            .map((entry) => `<li>${escapeHtml([entry.name, entry.issuer, entry.date].filter(Boolean).join(" · "))}</li>`)
            .join("")}</ul>`
        : "";
    return `<!doctype html>
<html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>${escapeHtml(resume.contact.name || "Tailored Resume")}</title><style>@page{margin:.6in}body{font:11pt/1.35 system-ui,sans-serif;max-width:8.5in;margin:auto;color:#111}h1{font-size:22pt;margin-bottom:4px}h2{font-size:12pt;border-bottom:1px solid #777;padding-bottom:2px;margin-top:18px}h3{font-size:11pt;margin:10px 0 2px}p{margin:3px 0}ul{margin:4px 0 8px;padding-left:18px}.contact{font-size:9.5pt}</style></head><body><header><h1>${escapeHtml(resume.contact.name)}</h1><p class="contact">${escapeHtml([resume.contact.email, resume.contact.phone, resume.contact.location, resume.contact.website]
        .filter(Boolean)
        .join(" · "))}</p></header>${section("Summary", `<p>${escapeHtml(resume.summary)}</p>`)}${section("Skills", resume.skills.length ? `<p>${resume.skills.map(escapeHtml).join(" · ")}</p>` : "")}${section("Experience", experience)}${section("Projects", projects)}${section("Education", education)}${section("Certifications", certifications)}</body></html>\n`;
}
function compileApplicationArtifacts(source, target, tailored, compiledAt = new Date().toISOString()) {
